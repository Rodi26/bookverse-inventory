...
engine = create_engine(
    DATABASE_URL,                              # Database connection string from configuration
    connect_args={"check_same_thread": False}, # Allow SQLite usage across multiple threads (FastAPI requirement)
)

# Create session factory with explicit transaction control for web request patterns
...
def load_demo_data():
    ...
    try:
        ...
        for book_data in books_data:
            book = Book(
                id=str(uuid4()),                                # Unique identifier for distributed systems
                title=book_data["title"],                       # Book title for display and search
                subtitle=book_data.get("subtitle"),             # Optional subtitle for detailed information
                authors=book_data["authors"],                   # Author information (may be list or string)
                genres=book_data["genres"],                     # Genre classifications for categorization
                description=book_data["description"],           # Detailed description for product pages
                price=Decimal(str(book_data["price"])),        # Precise decimal pricing for financial accuracy
                cover_image_url=book_data["cover_image_url"],  # Image URL for frontend display
                rating=Decimal(str(book_data["rating"])) if book_data.get("rating") else None  # Optional rating data
            )
        ...
        for i, inventory_item in enumerate(inventory_data):
            if i < len(book_objects):  # Ensure we have a corresponding book
                inventory = InventoryRecord(
                    id=str(uuid4()),                                      # Unique inventory record identifier
                    book_id=book_objects[i].id,                          # Foreign key to book record
                    quantity_available=inventory_item["quantity_available"],  # Current available stock
                    quantity_total=inventory_item["quantity_total"],      # Total inventory quantity
                    reorder_point=inventory_item.get("reorder_point", 5) # Low stock alert threshold
                )
...
def _create_demo_transactions(db: Session, books: List[Book]):
    ...
    transactions = [
        ...
        StockTransaction(
            ...
            quantity_change=5,                  # Adjustment quantity (can be positive or negative)
