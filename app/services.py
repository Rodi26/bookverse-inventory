...
class BookService:
    ...
    def get_books_with_availability(
        ...
        if not include_inactive:
            query = query.filter(Book.is_active)
        ...
        if book_ids:
            ...
            inventory_by_book_id = {
                inv.book_id: inv for inv in inventory_records}
    ...
    def get_book_by_id(db: Session, book_id: UUID) -> Optional[Book]:
        ...
        return db.query(Book).filter(
            and_(Book.id == str(book_id), Book.is_active)
        ).first()
    ...
    def create_book(db: Session, book_data: BookCreate) -> Book:
        ...
        # Atomic transaction ensures both book and inventory are created
        # together
        with db.begin():
            db.add(book)
    ...
    def update_book(
            db: Session,
            book_id: UUID,
            book_data: BookUpdate) -> Optional[Book]:
        """
...
class InventoryService:
    ...
    def adjust_inventory(
        ...
        if new_available < 0:
            logger.warning(
                f"Adjustment would result in negative available quantity for book {book_id}")
            return None
        if new_total < 0:
            logger.warning(
                f"Adjustment would result in negative total quantity for book {book_id}")
            return None
