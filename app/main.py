...
# Configure structured logging with request correlation IDs for debugging and monitoring
log_config = LogConfig(
    level=os.getenv("LOG_LEVEL", "INFO"),  # Support DEBUG, INFO, WARNING, ERROR levels
    include_request_id=True                # Enable request correlation across service boundaries
)
setup_logging(log_config, "inventory")

# Initialize logger for this module with consistent formatting and correlation ID support
logger = get_logger(__name__)

# Configure base service settings with environment-specific overrides
config = BaseConfig(
    service_name="inventory",                                           # Service identifier for monitoring
    environment=os.getenv("ENVIRONMENT", "development"),              # Runtime environment context
    auth_enabled=os.getenv("AUTH_ENABLED", "true").lower() == "true", # Authentication middleware toggle
)

# Determine service version from environment with fallback to configured default
...
# Create FastAPI application with comprehensive configuration for production readiness
app = create_app(
    title=SERVICE_NAME,                    # Human-readable service name for API documentation
    version=service_version,               # Service version for API versioning and monitoring
    description=SERVICE_DESCRIPTION,       # Service description for OpenAPI documentation
    config=config,                         # Base configuration with environment-specific settings
    enable_auth=config.auth_enabled,       # Conditional authentication middleware based on environment
    enable_cors=True,                      # Enable CORS for web application integration
    health_checks=["basic", "auth"],       # Configure health check types for monitoring
    lifespan=lifespan,                     # Lifecycle management for startup/shutdown operations
    middleware_config={
        "cors": {
            ...
            "allow_methods": ["*"],        # Allow all HTTP methods for flexibility
            "allow_headers": ["*"],        # Allow all headers for client compatibility
        },
        "logging": {
            "log_request_body": False,     # Disable request body logging for performance and privacy
...
# Add comprehensive request/response logging middleware for operational monitoring
...
# Configure static file serving for product images and assets (development/testing)
...
# Configure comprehensive health check endpoints for monitoring and load balancing
health_router = create_health_router(
    service_name="BookVerse Inventory Service",  # Service name for health check identification
    service_version=service_version,             # Version information for deployment tracking
    health_checks=["basic", "auth"]              # Enable basic service and authentication health checks
)
# Mount health router with standard /health prefix for monitoring tool compatibility
