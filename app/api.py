...
async def health_check(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        log_error_with_context(
            logger, e, {
                "endpoint": "health_check", "component": "database"})
...
async def get_current_user_info(user: AuthUser = RequireUser):
    ...
    """
    # Verify user authentication (should not be None due to RequireUser
    # dependency)
    if user is None:
...
async def update_book(
    ...
    try:
        ...
        book = BookService.update_book(
            db=db, book_id=book_id, book_data=book_data)
...
async def list_inventory(
    ...
    try:
        ...
        return InventoryListResponse(
            inventory=inventory, pagination=pagination)
...
@router.get("/api/v1/inventory/{book_id}",
...
async def get_book_inventory(book_id: UUID, db: Session = Depends(get_db)):
    ...
    try:
        ...
        inventory_data = InventoryService.get_inventory_by_book_id(
