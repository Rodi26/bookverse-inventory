...
def setup_logging(config: LogConfig = None,
                  service_name: str = "bookverse") -> None:
    ...
    logging.info(
        f"✅ Logging configured for {service_name} (level: {config.level})")
...
def log_duplication_eliminated(
        logger: logging.Logger,
        component: str,
        lines_saved: int):

    logger.info(
        f"♻️ COMMONS: {component} - eliminated {lines_saved} lines of duplicate code")
